...
- **Endpunkt (bestätigt):** `POST /split`
- **Request-Body (JSON)** — Felder `amount` (= `billAmount`), `tipPercent`, `people` (= `peopleCount`):
...
- **Response-Body (JSON, HTTP 200)** — kein Echo der Eingaben, nur `perPerson` und `totalTip`:
...
- Der Controller ist dünn: Validierung + Delegation an den Application-Service + Mapping auf HTTP. Feldnamen-Mapping (`amount`→`billAmount`, `people`→`peopleCount`) geschieht an der Api/Application-Grenze; die Domain behält ihre sprechenden internen Namen.
...
- [x] Bei `billAmount = 100`, `tipPercent = 0`, `peopleCount = 4` liefert die Berechnung `tipAmount = 0.00`, `totalAmount = 100.00`, `perPerson = [25.00, 25.00, 25.00, 25.00]`.
- [x] Bei `billAmount = 100`, `tipPercent = 10`, `peopleCount = 4` liefert die Berechnung `tipAmount = 10.00`, `totalAmount = 110.00`, `perPerson = [27.50, 27.50, 27.50, 27.50]`.
- [x] Bei `billAmount = 100`, `tipPercent = 0`, `peopleCount = 3` gilt `perPerson = [33.34, 33.33, 33.33]` — Rest-Cent liegt bei der ersten Person, `sum(perPerson) == 100.00`.
- [x] Bei `billAmount = 100`, `tipPercent = 0`, `peopleCount = 7` gilt `perPerson = [14.32, 14.28, 14.28, 14.28, 14.28, 14.28, 14.28]` — die 4 Rest-Cents liegen **vollständig** bei der ersten Person (`14.28 + 0.04`), `sum(perPerson) == 100.00`.
- [x] Bei `billAmount = 87.50`, `tipPercent = 10`, `peopleCount = 3` gilt `perPerson = [32.09, 32.08, 32.08]`, `sum(perPerson) == totalAmount == 96.25`.
- [x] Für jede beliebige gültige Kombination gilt: `sum(perPerson) == totalAmount` (kein verlorener Cent). Abgedeckt durch die konkreten Testfälle oben (generischer Algorithmus, kein Property-based-Test).
- [x] Für jede beliebige gültige Kombination trägt **Index 0** den vollen Rest: `perPerson[0] == basePerPerson + remainderCents/100`, alle übrigen Einträge sind exakt `basePerPerson`. Abgedeckt durch die konkreten Testfälle oben.
- [x] Jeder Wert in `perPerson` hat genau 2 Nachkommastellen (bzw. ist als `decimal` mit Scale 2 darstellbar).
- [x] Bei `billAmount = 0`, `tipPercent = 0`, `peopleCount = 4` gilt `tipAmount = 0.00`, `totalAmount = 0.00`, `perPerson = [0.00, 0.00, 0.00, 0.00]` (keine Exception).
- [x] Bei `billAmount = 0`, `tipPercent = 15`, `peopleCount = 3` gilt `tipAmount = 0.00`, `totalAmount = 0.00`, `perPerson = [0.00, 0.00, 0.00]` (keine Exception).
- [x] Bei `billAmount = 50`, `tipPercent = 0`, `peopleCount = 2` gilt `tipAmount = 0.00`, `totalAmount = 50.00`, `perPerson = [25.00, 25.00]` (0 % ist erlaubt). Implizit durch die anderen 0-%-Testfälle abgedeckt.
- [x] `peopleCount = 0` wirft eine Domain-Exception mit Meldung „Anzahl Personen muss mindestens 1 sein.“
- [x] `peopleCount = -1` wirft eine Domain-Exception mit derselben Meldung.
- [x] `billAmount = -0.01` wirft eine Domain-Exception mit Meldung „Rechnungsbetrag darf nicht negativ sein.“
- [x] `tipPercent = -0.01` wirft eine Domain-Exception mit Meldung „Trinkgeld-Prozentsatz darf nicht negativ sein.“

### Application

- [x] Der Application-Service liefert für gültige Eingaben ein Antwort-DTO, dessen Felder mit dem Domain-Ergebnis übereinstimmen.
- [x] Ungültige Eingaben schlagen mit einer erkennbaren Exception fehl, die vom Api-Layer in HTTP 400 übersetzt werden kann.

### API

- [x] `POST /split` mit gültigem JSON (`amount`, `tipPercent`, `people`) antwortet mit HTTP 200 und `{ perPerson, totalTip }`.
- [x] `POST /split` mit `people = 0` antwortet mit HTTP 400.
- [x] `POST /split` mit `amount = -1` antwortet mit HTTP 400.
- [x] `POST /split` mit `tipPercent = -5` antwortet mit HTTP 400.
- [x] `POST /split` mit fehlendem Feld antwortet mit HTTP 400.
- [x] Eine typische Anfrage wird in < 1 s beantwortet (Smoke-Test reicht; kein formelles Lasttest-Setup).

### Quality Gates

- [x] `dotnet build` läuft grün.
- [x] `dotnet test` läuft grün (19/19 Tests: alle neuen Tests aus dieser Spec + bestehender `ToolchainSmokeTests`).
...
- **HTTP-Contract** (2026-07-28, via `/plan`): Route ist `POST /split` (nicht `/api/split`). Request-Felder heißen `amount`, `tipPercent`, `people`. Response enthält nur `perPerson` und `totalTip` (kein Echo der Eingaben, kein `totalAmount`). Die Domain behält intern `billAmount`/`peopleCount` als sprechende Namen; das Feld-Mapping erfolgt an der Api/Application-Grenze.

## Resolved Decisions (Ergänzung nach Umsetzung)

- **Rundung des Trinkgelds** (2026-07-28, umgesetzt): Kaufmännische Rundung (`MidpointRounding.AwayFromZero`) auf 2 Nachkommastellen für `tipAmount`; die Restcent-Verteilung für `perPerson` arbeitet zusätzlich auf Cent-Ebene (`totalCents`), damit die Summe exakt aufgeht.
- **Trinkgeld-Basis** (2026-07-28, umgesetzt): Prozent bezieht sich auf `billAmount` (netto), wie ursprünglich vorgeschlagen. Keine Sonderfälle für „Trinkgeld auf inkl. Steuer/Service“ in dieser Iteration.
- **Fehlerformat** (2026-07-28, umgesetzt): `ProblemDetails` via `ControllerBase.Problem(...)`, HTTP 400 bei `TipSplitValidationException` und bei fehlenden Pflichtfeldern (`Amount`/`TipPercent`/`People` als nullable modelliert, `null` → 400).

## Open Questions (bewusst nicht in dieser Iteration adressiert)

- **Obergrenzen:** Gibt es sinnvolle obere Grenzen (z. B. `peopleCount <= 1000`, `billAmount <= 1_000_000`), die als Validierung erzwungen werden sollen? → nicht umgesetzt, kein Teil der Acceptance Criteria.
- **OpenAPI/Swagger:** Soll der Endpunkt im bereits vorhandenen OpenAPI-Setup (`Microsoft.AspNetCore.OpenApi`) sichtbar/dokumentiert sein? → noch nicht verifiziert/dokumentiert, eigene Folge-Task.
