...
- [ ] `POST /split` mit gültigem JSON (`amount`, `tipPercent`, `people`) antwortet mit HTTP 200 und `{ perPerson, totalTip }`.
- [ ] `POST /split` mit `people = 0` antwortet mit HTTP 400.
- [ ] `POST /split` mit `amount = -1` antwortet mit HTTP 400.
- [ ] `POST /split` mit `tipPercent = -5` antwortet mit HTTP 400.
- [ ] `POST /split` mit fehlendem Feld antwortet mit HTTP 400.
